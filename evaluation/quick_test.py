"""
Quick test with limited samples - faster validation
"""
...
def quick_test():
    """Run a quick test with only 10 HTML files."""
    ...
    output_dir = "/Users/brown/Projects/AILabProject/web2json-agent/swde_quick_test"

    vertical = "book"
    website = "abebooks"

    print("="*80)
    print("SWDE Quick Test - Processing only 10 samples")
    print("="*80)
    print(f"\nVertical: {vertical}")
    print(f"Website: {website}")
    print()

    # Create a temporary directory with only 10 HTML files
    source_dir = Path(dataset_dir) / "book" / f"book-{website}(2000)"
    temp_dir = Path(output_dir) / "temp_html"
    temp_dir.mkdir(parents=True, exist_ok=True)

    print(f"Copying 10 sample HTML files...")
    html_files = sorted(list(source_dir.glob("*.htm")))[:10]
    ...
    try:
        print("\n" + "="*80)
        print("Running agent on 10 HTML files...")
        print("="*80)
        ...
        print("="*80)
        print(f"\nResults for {vertical}/{website} (10 samples):")
        ...
        for attr, metrics in results['attribute_metrics'].items():
            print(f"  {attr:20s} - F1: {metrics['f1']:.2%}")
